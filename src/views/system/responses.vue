<template>
  <app-panel dark noerror>
    ...
    <template slot="button">
      ...
      <app-tooltip bottom :tooltip="view?'Mostrar Lista':'Mostrar Cuadricula'">
        <v-btn icon @click="c_view">
          <v-icon dark class="text-white">{{view?'fa-list':'fa-th'}}</v-icon>
        </v-btn>
      </app-tooltip>
    ...
    <v-card-text>
      <app-responses :items="responses" :search="search" :view="view"></app-responses>
    </v-card-text>
  </app-panel>
</template>
