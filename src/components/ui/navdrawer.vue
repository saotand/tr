<template>
  <span>
    <v-navigation-drawer
      ...
      app
    >
      <v-toolbar dense v-if="userdata">
        <v-toolbar-title v-if="!mini">
          <app-tooltip bottom tooltip="Usuario">{{user.name}}</app-tooltip>
        </v-toolbar-title>
        <v-spacer></v-spacer>
      ...
      <template v-for="(item, i) in items">
        ...
        <v-list-tile v-else :key="i" :to="item.link">
          <app-tooltip bottom :tooltip="item.title == 'USER'?user.name:item.title" v-if="mini">
            <v-list-tile-action>
      ...
      <app-tooltip bottom tooltip="Salir">
        <v-list-tile v-if="logoutshow" @click="userSignOut">
          <v-list-tile-action>
      ...
      <center>
        <app-tooltip bottom tooltip="Configuracion">
          <v-btn v-if="config" icon flat to="/system/config">
            <v-icon>fa-cogs</v-icon>
          </v-btn>
        </app-tooltip>
        <app-tooltip bottom tooltip="Minimizar Menú">
          <v-btn icon flat @click="mini = !mini">
