...
export default {
  ...
  data: () => ({
    form: {
      ...
      dialog: false,
      refs:
        "Las referencias (modelos) a esta marca no seran eliminadas y deben ser asignadas a otra marca para ser mostradas"
    }
  }),
  computed: {
    darkset() {
  ...
  methods: {
    onsubmit() {
      let ID = this.item.ID;
      this.$store.dispatch("admin_a_del_brands", ID);
      this.closewindow();
    },
    oncancel() {
      this.closewindow();
